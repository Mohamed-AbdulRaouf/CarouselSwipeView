//
//  CarouselSwipeViewManager.swift
//  CarouselSwipeView
//
//  Created by Raouf on 02/11/2024.
//

import UIKit

public protocol CarouselSwipeViewManagerDelegate: AnyObject {
...
public class CarouselSwipeViewManager: UIView, UIScrollViewDelegate {
    // MARK: - Variables
    public weak var delegate: CarouselSwipeViewManagerDelegate?
    ...
    private var adsLabel = UILabel()
    var orderNowTitleButton = ""
...
//        initWithAdsLabel()
    ...
    fileprivate func initWithCloseBttton() {
        closeButton.translatesAutoresizingMaskIntoConstraints = false
        closeButton.setImage(UIImage(named: "ic_close_circle"), for: .normal)
        let trailing = NSLayoutConstraint(item: closeButton, attribute: .trailing, relatedBy: .equal, toItem: self, attribute: .trailing, multiplier: 1, constant: -20)
        let top = NSLayoutConstraint(item: closeButton, attribute: .top, relatedBy: .equal, toItem: self, attribute: .top, multiplier: 1, constant: 20)
        let width = NSLayoutConstraint(item: closeButton, attribute: .width, relatedBy: .equal, toItem: self, attribute: .width, multiplier: 0, constant: 35)
        let height = NSLayoutConstraint(item: closeButton, attribute: .height, relatedBy: .equal, toItem: self, attribute: .height, multiplier: 0, constant: 35)
        
        self.insertSubview(closeButton, at: 0)
        self.bringSubviewToFront(closeButton)
        self.addConstraints([trailing, top, width, height])
    ...
    fileprivate func initWithAdsLabel() {
        let viewAds = UIView(frame: CGRect(x: 20, y: 20, width: 60, height: 35))
        viewAds.layer.borderWidth = 0.3
        viewAds.layer.borderColor = UIColor.gray.cgColor
        viewAds.layer.cornerRadius = 3
        adsLabel.translatesAutoresizingMaskIntoConstraints = false
        viewAds.addSubview(adsLabel)
        self.addSubview(viewAds)
        adsLabel.centerXAnchor.constraint(equalTo: viewAds.centerXAnchor).isActive = true
        adsLabel.centerYAnchor.constraint(equalTo: viewAds.centerYAnchor).isActive = true
        self.updateConstraints()
    }
    
    fileprivate func initWithOrderNowBttton() {
        orderNowButton.translatesAutoresizingMaskIntoConstraints = false
        orderNowButton.backgroundColor = UIColor(named: "AppTheme_BlueColor_#012CDA")
        orderNowButton.layer.cornerRadius = 8
        
        let center = NSLayoutConstraint(item: orderNowButton, attribute: .centerX, relatedBy: .equal, toItem: self, attribute: .centerX, multiplier: 1, constant: 0)
        let bottom = NSLayoutConstraint(item: orderNowButton, attribute: .bottom, relatedBy: .equal, toItem: self, attribute: .bottom, multiplier: 1, constant: -20)
        let width = NSLayoutConstraint(item: orderNowButton, attribute: .width, relatedBy: .equal, toItem: self, attribute: .width, multiplier: 0, constant: 200)
        let height = NSLayoutConstraint(item: orderNowButton, attribute: .height, relatedBy: .equal, toItem: self, attribute: .height, multiplier: 0, constant: 40)
        
        self.insertSubview(orderNowButton, at: 0)
        self.addConstraints([center, bottom, width, height])
        self.bringSubviewToFront(orderNowButton)
        orderNowButton.addTarget(self, action: #selector(buyOrderTapped(_:)), for: .touchUpInside)
    }
    
    @objc func buyOrderTapped(_ sender: UIButton) {
        self.delegate?.navigateForAdvertisements(index: self.currentIndex)
    }
    ...
    fileprivate func initWithPageControl() {
        ...
        let bottom = NSLayoutConstraint(item: pageControl, attribute: .bottom, relatedBy: .equal, toItem: self, attribute: .bottom, multiplier: 1, constant: 30)
    ...
    fileprivate func setImageViewFrame() {
        switch carouselMode {
        case .full:
            beforeImageView.frame = CGRect.init(x: scrollView.frame.size.width, y: 0, width: scrollView.frame.size.width, height: scrollView.frame.size.height - 50)
            currentImageView.frame = CGRect.init(x: scrollView.frame.size.width * 2, y: 0, width: scrollView.frame.size.width , height: scrollView.frame.size.height - 50)
            afterImageView.frame = CGRect.init(x: scrollView.frame.size.width * 3, y: 0, width: scrollView.frame.size.width, height: scrollView.frame.size.height - 50)
            break
        case .halfFull:
    ...
    public func setCarouselLayout(displayStyle:Int, pageIndicatorPositon:Int, pageIndicatorColor:UIColor?, describedTitleColor:UIColor?, layerColor:UIColor?) {
        ...
        layerView.backgroundColor = layerColor ?? .white
        setNeedsLayout()
    }
    ...
    public func setCarouselData(paths:[String],describedTitle:[String],isAutoScroll:Bool,timer:Double?,defaultImage:String?, adsTextLabel: String, orderNowTextButton: String) {
        if paths.count == 0 {
            return
        }
        timerInterval = timer
        ...
        self.adsLabel.text = adsTextLabel
        self.orderNowButton.setTitle(orderNowTextButton, for: .normal)
    }
...
// MARK: - Extension - AACarouselDelegate
extension CarouselSwipeViewManagerDelegate {
